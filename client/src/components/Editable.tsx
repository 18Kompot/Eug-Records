...
export default function Editable(props: IEditableProps) {
    ...
    function saveContent() {
        const res = postRequest(`pages/edit/${props.name}`, { content: content }, false);
        if (!res) {
            ...
            { !isEditing ? (
                ...
                <div>
                    <div className="row" style={{ height: "500px" }}>
                        <textarea onChange={ e => onContentChanged(e.target.value) } value={ content } />
                    </div>
                
                    <div className="row">
                        <div className="d-flex flex-col justify-content-end">
                            <button onClick={ toggleEdit } className="btn btn-danger me-2">Cancel</button>
                            <button onClick={ saveContent } className="btn btn-success">Save</button>
                        </div>
                    </div>
                </div>
            )}
